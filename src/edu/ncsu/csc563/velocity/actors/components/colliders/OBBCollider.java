...
public class OBBCollider extends Component {
	...
	public OBBCollider(Transform transform, float[] center, float[] halfWidths) {
		...
		for (int x = -1; x < 2; x += 2) {
			for (int y = -1; y < 2; y += 2) {
				for (int z = -1; z < 2; z += 2) {
					...
					float[] point = {x * this.mHalfWidths[0] + this.mCenter[0], y * this.mHalfWidths[1] + this.mCenter[1], z * this.mHalfWidths[2] + this.mCenter[2], 1};
